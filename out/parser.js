...
function parseTime(lineNum, timeToken, prevTime) {
    ...
    if (!isDelay) {
        ...
        if (prevTime === undefined) {
            return (0, error_1.error)(lineNum, "没有延迟基准, 请先使用非延迟语句", timeToken);
        }
        ;
...
function parseCob(out, lineNum, line, cobNum) {
    ...
    if (tl.length > 0) {
        return (0, error_1.error)(lineNum, "请删去多余的参数", tl);
    }
    let cobCol;
    if (/\d$/.test(symbol)) {
    ...
    const parseFodderArgs = (fodderArgTokens, cardNum, mustProvideChoose) => {
        ...
        for (const fodderArgToken of fodderArgTokens) {
            ...
            if (key === "choose") {
                ...
                return (0, error_1.error)(lineNum, "未知参数", `${key} (支持的参数: choose, waves)`);
            }
        }
    ...
    for (const zombieTypeAbbr of containChinese ? zombieTypeAbbrs : zombieTypeAbbrs.split(" ")) {
        ...
        if (containChinese) {
            ...
            if (parsedZombieType === undefined) {
                return (0, error_1.error)(lineNum, `未知僵尸类型`, `${zombieTypeAbbr} (支持的僵尸类型: ${Object.keys(zombie_types_1.zombieTypeCNAbbrToEnum)})`);
            }
    ...
    for (let { lineNum, line } of lines) {
        if (line.length > 0 && !line.startsWith("scene:")) {
            ...
            if (symbol.startsWith("protect:")) {
                ...
                parseResult = (0, error_1.error)(lineNum, "未知符号", `${symbol} (使用帮助: https://marketplace.visualstudio.com/items?itemName=Crescendo.seml)`);
            }
            if ((0, error_1.isError)(parseResult)) {
                return parseResult;
            }
