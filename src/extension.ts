...
export function activate(context: vscode.ExtensionContext) {
	...
	function error(lineNum: number, message: string, source: string) {
	...
	function parseWave(out: Output, lineNum: number, line: string) {
		const parseWaveNum = (waveNumToken: string) => {
		...
		const parseIceTimes = (iceTimeTokens: string[]) => {
			const iceTimes = [];
		...
		const parseWaveLength = (waveLengthToken: string) => {
		...
		const waveNumToken = tokens[0]!,
			iceTimeTokens = tokens.slice(1, tokens.length - 1),
	...
	function parseTime(lineNum: number, timeToken: string, prevTime: number | undefined) {
	...
	function parseRows(lineNum: number, rowsToken: string, expectedNum: number | null, suffix: string | null, description: string) {
		const rows = [];
	...
	function parseCob(out: Output, lineNum: number, line: string, currentWave: number, cobNum: number) {
		...
		const parseCol = (colToken: string) => {
	...
	function parseFodder(out: Output, lineNum: number, line: string, currentWave: number) {
		const wave = out[currentWave];
		if (!wave) {
			error(lineNum, "请先设定波次", line);
			return;
		...
		const parseTimes = (timesToken: string): [number, number | undefined] | undefined => {
			let cardTimeToken: string;
			let shovelTimeToken: string | undefined = undefined;
		...
		const parseExtraArgs = (extraArgTokens: string[], cardNum: number) => {
			if (extraArgTokens.length === 0) { return {}; }

			let extraArgs: CardExtraArgs = {};
